...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        int hashCompress = Math.abs(key.hashCode() % table.length);
        if (table[hashCompress] == null) {
            table[hashCompress] = new ExternalChainingMapEntry(key, value);
        } else {
            ExternalChainingMapEntry<K, V> curr = table[hashCompress];
            while (curr != null) {
                if (curr.getKey().equals(key)) {
                    V data = curr.getValue();
                    curr.setValue(value);
                    return data;
                }
                curr = curr.getNext();
            }
            table[hashCompress] = new ExternalChainingMapEntry(key, value, table[hashCompress]);
        }
        size++;
        return null;
    }
    ...
    public V remove(K key) {
        ...
        while (curr != null) {
            if (curr.getKey().equals(key)) {
                value = curr.getValue();
                if (prev == null) {
                    table[hashCompress] = curr.getNext();
                } else {
                    ExternalChainingMapEntry<K, V> pointer = curr.getNext();
    ...
    public V get(K key) {
        ...
        if (table[hashCompress] != null) {
            ...
            while (curr != null) {
                if (curr.getKey().equals(key)) {
                    value = table[hashCompress].getValue();
                }
                curr = curr.getNext();
            }
        }
    ...
    public void resizeBackingTable(int length) {
        ...
        ExternalChainingMapEntry<K, V>[] resize = new ExternalChainingMapEntry[length];
        for (int i = 0; i < table.length; i++) {
            if (table[i] != null) {
                ExternalChainingMapEntry<K, V> curr = table[i];
                while (curr != null) {
                    K newKey = curr.getKey();
                    V newValue = curr.getValue();
                    int newCompress = Math.abs(newKey.hashCode() % length);
                    if (resize[newCompress] == null) {
                        resize[newCompress] = new ExternalChainingMapEntry(newKey, newValue);
                    } else {
                        ExternalChainingMapEntry<K, V> pair =
                                new ExternalChainingMapEntry(newKey, newValue, resize[newCompress]);
                        resize[newCompress] = pair;
                    }
                    curr = curr.getNext();
                }
            }
